...
class QuizInterface:
    def __init__(self, quiz_brain: QuizBrain):
        ...
        self.score_label = Label(text=f"Score: {quiz_brain.score}", font=FONT, bg=THEME_COLOR, fg="white")
        self.score_label.grid(row=0, column=1)

        self.canvas = Canvas()
        self.canvas.config(width=300, height=250, bg="white")
        self.canvas_text = self.canvas.create_text(150, 125, width=280,text="Some Question: Test", font=FONT, fill=THEME_COLOR)
        ...
        self.true_btn.grid(row=2, column=0, padx=25, pady=25)
        self.false_btn = Button(image=false_img, highlightthickness=0, command=self.false_pressed)
        self.false_btn.grid(row=2, column=1, padx=25, pady=25)
